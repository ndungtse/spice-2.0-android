package org.medtroniclabs.uhis.app.analytics.upload
...
import org.medtroniclabs.uhis.app.analytics.model.AnalyticsDetail
import org.medtroniclabs.uhis.app.analytics.network.ApiService
...
class UploadWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    private suspend fun getAllAnalyticsData(): AnalyticsData? {
        ...
        val userAnalyticsMap = list
            ...
            .mapValues { (_, analyticsList) ->
                ...
                val analyticsMap = analyticsList
                ...
                val sessionTracking =
                    analyticsList
                        .filter { it.eventType == AnalyticsDefinedParams.SessionTracking }
                        .map { JsonParser.parseString(it.parameter) }
                        .toMutableList()
    ...
    private suspend fun uploadFileFilter(
        baseUrl: String,
        headers: String,
        buildConfig: String?,
    ) {
...
//            initializeApiService(baseUrl, header)
    ...
    private fun initializeApiService(
        baseUrl: String,
        header: String,
    ): ApiService = RetrofitHelper.initializeRetrofit(baseUrl, header)
