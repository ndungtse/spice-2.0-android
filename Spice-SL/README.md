...
## Prerequisite (Backend - Set up)
- To set up the Spice backend server, begin by cloning the backend repository. Next, configure and run the essential backend services. Once these services are successfully running on the server, obtain the backend server URL and update the relevant configuration fields in the application's build.gradle file accordingly.
...
Once you have Git installed, you can clone the Spice repository by running the following command in your terminal:

```sh
git clone git@bitbucket.org:MDTLabs/spice_mobile.git
```
</br>
After cloning the repository, open the Spice-SL folder in Android Studio.
...
Use the server URL obtained from the prerequisites mentioned above.
```server
...
By default, the Salt key is set as spice_uat, but you can modify it if necessary. Please note that the Salt key must match the key used in the backend.
