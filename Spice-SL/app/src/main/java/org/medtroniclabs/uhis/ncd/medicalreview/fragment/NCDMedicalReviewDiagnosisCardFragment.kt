...
import org.medtroniclabs.uhis.databinding.FragmentNcdMedicalReviewDiagnosisCardBinding
import org.medtroniclabs.uhis.formgeneration.extension.safeClickListener
import org.medtroniclabs.uhis.mappingkey.Screening
import org.medtroniclabs.uhis.ncd.assessment.dialog.BPLogListDialog
import org.medtroniclabs.uhis.ncd.assessment.dialog.GlucoseLogListDialog
...
import org.medtroniclabs.uhis.ncd.medicalreview.NCDMRUtil.IS_FEMALE
import org.medtroniclabs.uhis.ncd.medicalreview.NCDMRUtil.IS_INITIAL_MR
...
@AndroidEntryPoint
class NCDMedicalReviewDiagnosisCardFragment :
    BaseFragment(),
    ...
    private fun getInitialMr(): Boolean = arguments?.getBoolean(IS_INITIAL_MR) ?: false

    private fun getIsFemale(): Boolean = arguments?.getBoolean(IS_FEMALE) ?: false

    private fun getMenu(): String? = arguments?.getString(MENU_ID)?.lowercase()

    private fun geEncounterReference(): String? = arguments?.getString(EncounterReference)
    ...
    private fun isFemalePregnancy(): Boolean = getMenu().equals(DefinedParams.PregnancyANC, true) && getIsFemale()

    private fun isMentalHealth(): Boolean = getMenu().equals(NCDMRUtil.mentalHealth, true)
