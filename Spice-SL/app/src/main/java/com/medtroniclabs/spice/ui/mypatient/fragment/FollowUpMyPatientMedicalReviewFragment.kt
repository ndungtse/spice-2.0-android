...
class FollowUpMyPatientMedicalReviewFragment : BaseFragment(), FollowUpCommunicator {
    ...
    companion object {
        const val TAG = "FragmentFollowUpMyPatientMedicalReview"
    }
...
//                hideLoading()
                binding.apply {
                    tvPatientNoFound.visibility = if (resource.data?.isNotEmpty() == true) {
        ...
        with(binding) {
            rvPatientList.apply {
                adapter = PatientListAdapter(
                    ...
                    this@FollowUpMyPatientMedicalReviewFragment
                )
                layoutManager = GridLayoutManager(
                    context, viewModel.spanCount
                )
            }
        }
    }
