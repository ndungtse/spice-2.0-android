...
class NCDComplicationsFragment : BaseFragment() {
    ...
    fun validateInputCommon(
        chips: List<ChipViewItemModel>, // pass chips from the viewModel
        comments: String?, // pass comments from the view
        isMandatory: Boolean = false,
        errorMessageView: View, // pass the error message view like binding.tvErrorMessage
        titleView: TextView // pass the title view like binding.tvSystemicExaminationTitle
    ): Boolean {
        val hasChips = chips.isNotEmpty() // Check if there are any chips selected
        val hasOtherChip = chips.any { it.name.equals(DefinedParams.Other, ignoreCase = true) } // Check if 'Other' chip is present
        val commentsNotBlank = comments?.isNotBlank() == true // Check if the comments are not blank

        // If input is mandatory, additional validation is required
        if (isMandatory) {
            titleView.markMandatory() // Apply mandatory styling to the title

            // If there are chips, we need to check further
            if (hasChips) {
                // If 'Other' chip is selected, check for non-blank comments
                return if (hasOtherChip) {
                    if (commentsNotBlank) {
                        errorMessageView.invisible() // Hide error if comments are valid
                        true
                    } else {
                        errorMessageView.visible() // Show error if comments are empty
                        false
                    }
                } else {
                    errorMessageView.gone() // Hide error if no 'Other' chip is selected
                    true
                }
            } else {
                // If no chips are selected and mandatory, show error
        ...
        // If chips are empty and comments are blank, input is valid
        if (!hasChips && comments?.isBlank() == true) {
